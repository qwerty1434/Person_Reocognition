class argHandler(dict):
    ...
    def setDefaults(self):
        self.define('imgdir', './photos/', 'path to testing directory with images')
        ...
        self.define('model', './cfg/yolo.cfg', 'configuration of choice')#change default
        self.define('trainer', 'rmsprop', 'training algorithm')
        self.define('momentum', 0.0, 'applicable for rmsprop and momentum optimizers')
        self.define('verbalise', False, 'say out loud while building graph')#True에서 False로 바꿨다 True면 망의 구조를 보여준다
        self.define('train', False, 'train the whole net')
        self.define('load', './bin/yolo.weights', 'how to initialize the net? Either from .weights or a checkpoint, or even from scratch')#./bin/yolo.weights
        self.define('savepb', False, 'save net and weight to a .pb file')
        self.define('gpu', 0.5, 'how much gpu (from 0.0 to 1.0)')
